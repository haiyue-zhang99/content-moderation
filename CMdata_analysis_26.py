...
# 页面设置
# =========================
st.set_page_config(page_title="🧑‍💻 内容审核数据统计工具", layout="wide")
...
if selected == "审核数据统计":
    st.markdown("""
### 📊 审核数据统计
上传审核数据（支持 .xlsx / .csv），平台即刻自动执行多维度统计分析，并生成可视化成果。
""", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
        ...
        email_order = [
            ...
            "v-cwen@microsoft.com", "v-yuehan@microsoft.com", "v-shuagao@microsoft.com",
            "v-xiyuan1@microsoft.com", "v-xuelyang@microsoft.com", "v-wenlchen@microsoft.com",
            "v-huiwwang@microsoft.com", "v-dandanli@microsoft.com", "v-yuanjunli@microsoft.com",
            "v-yuqincheng@microsoft.com", "v-jiaozhang@microsoft.com", "v-wangjua@microsoft.com",
            ...
            "v-chaozhao@microsoft.com", "v-junlanli@microsoft.com", "v-tengpan@microsoft.com",
            "v-qinjiang@microsoft.com", "v-yancche@microsoft.com", "v-jiangli@microsoft.com",
            "v-yuleiwu@microsoft.com", "v-liuyang@microsoft.com", "v-weihu1@microsoft.com",
            "v-haizhang@microsoft.com", "v-xiaoqingwu@microsoft.com", "v-lixren@microsoft.com",
        ]
        ...
        for email in email_order:
            ...
            # 简单列表统计
            simple_mask = df1['RankList'].astype(str).str.contains('简单列表', na=False)
            simple_count = user_data[simple_mask].shape[0]

            # 一般/优质/视频列表统计（合并）
            other_mask = df1['RankList'].astype(str).str.contains('一般列表|优质列表|视频列表', na=False)
            other_count = user_data[other_mask].shape[0]
...
""", unsafe_allow_html=True)
        ...
        # ---------- 3. 被拒次数最多的作者 Top 20 ----------
        st.markdown("""
##### 📌 第三项：被拒次数最多的作者 Top 20
统计被拒次数最多的作者排名。
""", unsafe_allow_html=True)

        rejected_authors = (
            df1[df1['Action'] == 'Rejected']['Author']
            .value_counts()
            .head(20)
            .reset_index()
        )
...
""", unsafe_allow_html=True)
        ...
        st.dataframe(
            result_df1.style.set_properties(**{'font-size': '12px'}).format("{:.2f}%"),
...
""", unsafe_allow_html=True)
        ...
        if input_text and len(input_text) >= 2:
            matching_brands = [brand for brand in all_providers if input_text.lower() in str(brand).lower()]
            if matching_brands:
...
""", unsafe_allow_html=True)
        ...
        stopwords = set([
            "的", "了", "和", "是", "我", "也", "就", "都", "而", "及", "与", "着", "或", "一个", "没有",
            "我们", "你", "他", "她", "它", "在", "上", "下", "中", "为", "对", "不", "这", "那",
            "：", "，", "。", "！", "？", "（", "）", "、", "；", "“", "”", "‘", "’", "【", "】", "《", "》",
            " ", "\n", "\r", "\t"
        ])
        ...
        if word_freq:
            # Windows 可用 msyh.ttc；Linux 环境请替换为系统存在的中文字体路径
...
# =========================
    ...
    st.markdown("""
### 📈 编辑加分统计
上传编辑审核数据（支持 .xlsx / .csv），加分结果即刻自动计算呈现。
""", unsafe_allow_html=True)

    uploaded_file2 = st.file_uploader("📂 Upload file（.xlsx 或 .csv）", type=["xlsx", "csv"], key="file2")

    if uploaded_file2 is not None:
        try:
            # 仅校验“审核人员”，其余天字段动态判定
            df2 = load_dataframe(uploaded_file2, required_cols=["审核人员"])
            # 动态判断所有天的“简单列表数量”或“一般+优质列表数量”是否存在至少一列
            day_cols = [col for col in df2.columns if ("简单列表数量" in col) or ("一般+优质列表数量" in col)]
            if not day_cols:
                raise ValueError("上传文件不符合要求，缺少“简单列表数量”或“一般+优质列表数量”相关字段，请重新上传。")
    ...
    if st.session_state["df2"] is not None:
        ...
        # 解析“天”集合：根据列名中出现的“简单列表数量”抽取前缀为 day
        days = []
        ...
        st.dataframe(
            wide_df.style.applymap(color_score, subset=score_cols).set_properties(**{'font-size': '13px'}),
            use_container_width=True
        )

        # 下载按钮（Excel）
        st.markdown("#### ⬇️ 下载周度横向宽表加分结果")
        out_name = "周度横向宽表加分结果.xlsx"
        wide_df.to_excel(out_name, index=False)
        with open(out_name, "rb") as file:
            st.download_button("⬇️ 点此下载", data=file, file_name=out_name)
