...
if uploaded_file and st.button("🔄 生成排班表"):
    ...
    if len(editors) < 30:
        st.error("编辑人数必须为 30 人，才能确保早班、晚班、常规班各 10 人且不重叠。")
    elif weeks_to_schedule * 5 > len(editors):
        st.error("编辑人数不足以平均分配每周 5 人的周末值班，请增加人员或减少排班周数。")
    else:
        ...
        weekend_groups = [shuffled_editors[i:i+5] for i in range(0, weeks_to_schedule * 5, 5)]
        ...
        for week in range(weeks_to_schedule):
            ...
            used_this_week = set()
            morning = [e for e in get_sorted_candidates("早班", used_this_week)[:10]]
            used_this_week.update(morning)
            evening = [e for e in get_sorted_candidates("晚班", used_this_week)[:10]]
            used_this_week.update(evening)
            regular = [e for e in get_sorted_candidates("常规班", used_this_week)[:10]]
            used_this_week.update(regular)

            if len(morning) < 10 or len(evening) < 10 or len(regular) < 10:
                st.error(f"第 {week+1} 周排班失败：无法分配三班各 10 人且不重叠。")
                st.stop()
