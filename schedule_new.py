        ...
        if morning_count + evening_count > 30:
            ...
            for week in range(weeks_to_schedule):
                ...
                def get_sorted_candidates(shift_type, exclude_set):
                    ...
                    if shift_type in ["早班", "晚班"] and week > 0:
                        already_assigned = [e for e in used_this_week if e in weekend_last_week]
                        if len(already_assigned) >= 2:
                            candidates = [e for e in candidates if e not in weekend_last_week]
                ...
                weekend_candidates = [e for e in editors if e not in exclude_weekend and is_eligible_for_weekend(e, week)]
                if len(weekend_candidates) < weekend_count:
                    st.error(f"第 {week+1} 周周末值班人选不足，请调整排除名单或減少排班周数。")
                    st.stop()

                weekend_assigned = weekend_candidates[:weekend_count]
                for d in weekend:
                ...
                weekend_last_week = set(weekend_assigned)
