...
if selected == "审核数据统计":
    ...
    uploaded_file1 = st.file_uploader("📂 请上传脱敏审核数据（.xlsx）", type=["xlsx"], key="file1")
    if uploaded_file1 is not None:
        st.session_state["uploaded_file1"] = uploaded_file1
        st.session_state["df1"] = pd.read_excel(uploaded_file1, engine='openpyxl')
...
上传编辑审核数据，加分结果即刻自动计算呈现。
    ...
    uploaded_file2 = st.file_uploader("📂 请上传编辑审核数据（.xlsx）", type=["xlsx"], key="file2")
    if uploaded_file2 is not None:
        st.session_state["uploaded_file2"] = uploaded_file2
        st.session_state["df2"] = pd.read_excel(uploaded_file2, engine='openpyxl')
        ...
        for _, row in df2.iterrows():
            ...
            for day in days:
                ...
                if complex_valid and not simple_valid:
                    if complex_avg >= 162.5:
                        score = 1
                    elif complex_avg < 156.25:
                        score = -1
                    ...
                    if simple_avg >= 350 and complex_avg >= 162.5:
                        score = 1
                    elif simple_avg < 320 or complex_avg < 156.25:
                        score = -1
                total_score += score
